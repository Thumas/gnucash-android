...
/**
 * Fragment for creating transactions without set source account.
 * @author Thomas Krause <thumas@mailbox.org>
 */
public class FreeTransactionFormFragment extends Fragment implements
    ...
    /**
     * Adapter for source account spinner
     */
    private QualifiedAccountNameCursorAdapter mSourceAccountCursorAdapter;

	/**
	 * Adapter for transfer account spinner
	 */
	private QualifiedAccountNameCursorAdapter mTransferAccountCursorAdapter;

    /**
     * Cursor for source account spinner
     */
    private Cursor mSourceAccountCursor;

	/**
	 * Cursor for transfer account spinner
	 */
	private Cursor mTransferAccountCursor;
	 ...
	 */
	@BindView(R.id.input_free_transaction_name) AutoCompleteTextView mDescriptionEditText;

	/**
	 * Input field for the transaction amount
	 */
	@BindView(R.id.input_free_transaction_amount) CalculatorEditText mAmountEditText;

	/**
	 * Field for the transaction currency.
	 * The transaction uses the currency of the account
	 */
	@BindView(R.id.free_transaction_currency_symbol) TextView mCurrencyTextView;

	/**
	 * Input field for the transaction description (note)
	 */
	@BindView(R.id.free_transaction_input_description) EditText mNotesEditText;

	/**
	 * Input field for the transaction date
	 */
	@BindView(R.id.free_transaction_input_date) TextView mDateTextView;

	/**
	 * Input field for the transaction time
	 */
	@BindView(R.id.free_transaction_input_time) TextView mTimeTextView;

    /**
     * Spinner for selecting the source account
     */
    @BindView(R.id.free_transaction_input_source_account_spinner)
    SearchableSpinnerView mSourceAccountSearchableSpinnerView;

	/**
	 * Spinner for selecting the transfer account
	 */
    @BindView(R.id.free_transaction_input_transfer_account_spinner)
     ...
     */
    @BindView(R.id.free_transaction_checkbox_save_template) CheckBox mSaveTemplateCheckbox;

    @BindView(R.id.free_transaction_input_recurrence) TextView mRecurrenceTextView;

    /**
     * View which displays the calculator keyboard
     */
    @BindView(R.id.free_transaction_calculator_keyboard) KeyboardView mKeyboardView;

    /**
     * Open the split editor
     */
    @BindView(R.id.free_transaction_btn_split_editor) ImageView mOpenSplitEditor;

    /**
     * Layout for source account and associated views
     */
    @BindView(R.id.layout_double_entry_account_1) View mDoubleEntryAccountOneLayout;

    /**
     * Layout for transfer account and associated views
     */
    @BindView(R.id.layout_double_entry_account_2) View mDoubleEntryAccountTwoLayout;
    ...
    /**
     * Convenience method for determining the currently selected source account cursor id.
     */
    private long getSourceAccountId() {
        if (mSourceAccountSearchableSpinnerView != null) {
            return mSourceAccountSearchableSpinnerView.getSelectedItemId();
        } else {
    ...
    /**
     * Convenience method for determining the currently selected transfer account  cursor id.
     */
    private long getTransferAccountId() {
        if (mTransferAccountSearchableSpinnerView != null) {
            return mTransferAccountSearchableSpinnerView.getSelectedItemId();
        } else {
            // TODO: or should we throw an exception?
            return 0;
        }
    ...
    /**
     * Initialization before creating the view.
     */
    @Override
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        View v = inflater.inflate(R.layout.fragment_free_transaction_form,
                                  container,
                                  false);
        ...
        // initialize account spinners
        updateSourceAccountsList();
        updateTransferAccountsList();

        mSourceAccountSearchableSpinnerView.setTitle(getString(R.string.select_account));

        mSourceAccountSearchableSpinnerView.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            /**
            ...
            public void onItemSelected(AdapterView<?> adapterView,
                ...
                if (mSplitsList.size() == 2) {
                    ...
                    for (Split split : mSplitsList) {
                        ...
                        if (split.getAccountUID().equals(getSourceAccountUID())) {
...
//            mDescriptionEditText.clearFocus();
    ...
    private void startTransferFunds() {
        Commodity fromCommodity = Commodity.getInstance((mTransactionsDbAdapter.getAccountCurrencyCode(getSourceAccountUID())));
    ...
    private void initTransactionNameAutocomplete() {
        ...
        adapter.setFilterQueryProvider(new FilterQueryProvider() {
            ...
            public Cursor runQuery(CharSequence name) {
                return mTransactionsDbAdapter.fetchTransactionSuggestions(name == null ? "" : name.toString(), getSourceAccountUID());
            }
        });
	...
	private void initializeViewsWithTransaction(){
        ...
        // first split belongs to the source account
        final String sourceAccountUID = mSplitsList.get(0).getAccountUID();
        setSelectedSourceAccount(mAccountsDbAdapter.getID(sourceAccountUID));

        if (mSplitsList.size() == 2){
            for (Split split : mSplitsList) {
                if (split.getAccountUID().equals(sourceAccountUID)) {
        ...
        if (mSplitsList.size() == 2 && mSplitsList.get(0).isPairOf(mSplitsList.get(1))) {
            ...
            for (Split split : mTransaction.getSplits()) {
                ...
                if (mUseDoubleEntry && !split.getAccountUID().equals(sourceAccountUID)) {
        ...
        if (!mAmountEditText.isInputModified()) {
            ...
            final Money signedTransactionBalance = Transaction.computeBalance(sourceAccountUID,
                                                                              mSplitsList);

            updateAmountEditText(signedTransactionBalance);
        }

        String    currencyCode     = mTransactionsDbAdapter.getAccountCurrencyCode(sourceAccountUID);
    ...
    private void updateAmountEditText(final Money signedTransactionBalance) {
        ...
        //
        AccountType accountType = getSourceAccountType();
    ...
    private void setDoubleEntryViewsVisibility(int visibility) {
        mDoubleEntryAccountTwoLayout.setVisibility(visibility);
     ...
     */
    private void updateSourceAccountsList(){

        // Get Accounts that are not hidden, nor Placeholder, nor root Account
        String where = AccountUtils.getTransfertAccountWhereClause(null);

        if (mSourceAccountCursor != null) {
            mSourceAccountCursor.close();
        }

        mSourceAccountCursor = mAccountsDbAdapter.fetchAccountsOrderedByFavoriteAndFullName(where,
                null);
        ...
        mSourceAccountSearchableSpinnerView.setAdapter(mSourceAccountCursorAdapter);
    }
	...
	private void updateTransferAccountsList(){
        ...
        // get Accounts that are not hidden, nor Placeholder, nor root Account, nor the source account
        String where = AccountUtils.getTransfertAccountWhereClause(getSourceAccountUID());

        if (mTransferAccountCursor != null) {
            mTransferAccountCursor.close();
        }

        mTransferAccountCursor = mAccountsDbAdapter.fetchAccountsOrderedByFavoriteAndFullName(where,
                                                                               null);

        mTransferAccountCursorAdapter = new QualifiedAccountNameCursorAdapter(getActivity(),
                mTransferAccountCursor,
                                                                      where,
                                                                      null,
                                                                      R.layout.account_spinner_dropdown_item);

        mTransferAccountSearchableSpinnerView.setAdapter(mTransferAccountCursorAdapter);
	}
    ...
    private void setListeners() {
        ...
        mTransactionTypeSwitch.addOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView,
                ...
                final Money signedTransactionBalance = transaction.getBalance(getSourceAccountUID());
		...
		mDateTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CalendarDatePickerDialogFragment datePickerDialog = new CalendarDatePickerDialogFragment()
                        .setOnDateSetListener(FreeTransactionFormFragment.this)
		...
		mTimeTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                RadialTimePickerDialogFragment timePickerDialog = new RadialTimePickerDialogFragment()
                        .setOnTimeSetListener(FreeTransactionFormFragment.this)
    ...
    /**
     * Updates the spinner to the selected source account
     * @param accountId Database ID of the source account
     */
    private void setSelectedSourceAccount(long accountId){
        final String accountUID = mAccountsDbAdapter.getUID(accountId);

        int position = mSourceAccountCursorAdapter.getPosition(accountUID);
        if (position >= 0)
	...
	private void setSelectedTransferAccount(long accountId){
        ...
        int position = mTransferAccountCursorAdapter.getPosition(accountUID);
        if (position >= 0)
            mTransferAccountSearchableSpinnerView.setSelection(position);
    }
    ...
    private List<Split> extractSplitsFromView() {
        ...
        final String               transferAcctUID = getTransferAccountUID();
        ...
        if (mSplitsList.size() > 2) {
            ...
            split1.setAccountUID(sourceAccountUID);
    ...
    /**
     * Returns the GUID of the currently selected source account.
     * @return GUID of source account
     */
    private @NonNull String getSourceAccountUID() {
        long sourceAccountId = getSourceAccountId();
        return mAccountsDbAdapter.getUID(sourceAccountId);
    }

    /**
     * Returns the type of the currently selected source account.
     * @return GUID of source account
     */
    private @NonNull AccountType getSourceAccountType() {
        long sourceAccountId = getSourceAccountId();
        return mAccountsDbAdapter.getAccountType(sourceAccountId);
    }
    ...
    private @NonNull String getTransferAccountUID() {
        ...
        if (mUseDoubleEntry) {
            ...
            Commodity baseCommodity = mAccountsDbAdapter.getRecord(getSourceAccountUID()).getCommodity();
    ...
    private @NonNull Transaction extractTransactionFromView(){
        ...
        String currencyCode = mAccountsDbAdapter.getAccountCurrencyCode(getSourceAccountUID());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_save:
            if (canSave()){
                ...
                if (mSourceAccountSearchableSpinnerView.getCount() == 0){
                    Toast.makeText(getActivity(),
                            R.string.toast_specify_source_account_to_save_transaction,
    ...
    private boolean canSave(){
        return (mAmountEditText.isInputValid() && mSourceAccountSearchableSpinnerView.getCount() > 0
                && (!mUseDoubleEntry || mTransferAccountSearchableSpinnerView.getCount() > 0));
    }
    ...
    public void setSplitList(List<Split> splitList){
        ...
        Money balance = Transaction.computeBalance(getSourceAccountUID(), mSplitsList);
